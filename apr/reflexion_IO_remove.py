...
def create_problem_template(item: dict, include_buggy_code: bool = True) -> str:
    ...
    sample inputs: {item["prob_desc_sample_inputs"]}

    sample outputs: {item["prob_desc_sample_outputs"]}

    Notes: {item["prob_desc_notes"]}
...
def generate_function(
    ...
    problem_context = create_problem_template(item, include_buggy_code=True)
...
def run_single_item(
    ...
    reflection = gen.first_reflection(
        problem_context=create_problem_template(item, True),
        # inferred_specificaion=inferred_specificaion,
    ...
    for cur_impl in func_impls:
        ...
        print(f"cur_feedback: {cur_feedback}")

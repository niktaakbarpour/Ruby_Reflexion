...
int main() {
    ...
    if (x > 0 && y > 0) {
        ...
        // Fourth quadrant: x positive, y negative.
        int L = x - y;  // y is negative, so L is positive.
        cout << "0 " << -L << " " << L << " 0" << endl;
    }
    else if (x < 0 && y > 0) {
        // Second quadrant: x negative, y positive.
        int L = y - x;  // x is negative, so L is positive.
        cout << -L << " 0 0 " << L << endl;
    }
    else {
...
int main() {
    ...
    for (int val : values) {
        formatted_values = formatted_values + to_string(val) + ", ";
        total = val;
    }

    cout << "Total: " << total << ", Values: " << formatted_values << endl;
    return 0;
}
```
...
formatted_values = formatted_values + to_string(val) + ", " always appends a trailing comma, even at the last element.
...
Step-by-step plan:
1. Initialize total ← 0 and formatted_values ← empty list of strings.
2. For each element val in values:
2.1. Add val to total.
2.2. Append val (as string) to formatted_values list.
3. After the loop, create a joined_values string by joining all elements in formatted_values using commas.
4. Print "Total: <total>, Values: <joined_values>".
...
int main() {
    vector<int> values = {5, 10, 15};
    int total = 0;
    vector<string> formatted_values;

    for (int val : values) {
        total += val;  // Calculate the total sum
        formatted_values.push_back(to_string(val));  // Convert to string and add to list
    }

    string joined_values = "";
    for (size_t i = 0; i < formatted_values.size(); ++i) {
        joined_values += formatted_values[i];
        if (i != formatted_values.size() - 1) {
            joined_values += ", ";
        }
    }

    cout << "Total: " << total << ", Values: " << joined_values << endl;
...
[self-reflection]:
while i < matrix.size() && j < matrix[0].size() ensures the loop exits before iterating all diagonals.
...
**1. Basic Test Cases**:
- **Objective**: Verify the program’s fundamental correctness under standard conditions.
